...
class AADSSO_AuthorizationHelper
{
	...
	/**
	 * @var string List of scopes will be granted by sign-in user.
	 */
	private static $allowed_scopes = "App.Read.All Capacity.Read.All Capacity.ReadWrite.All Content.Create Dashboard.Read.All Dashboard.ReadWrite.All Data.Alter_Any Dataflow.Read.All Dataflow.ReadWrite.All Dataset.Read.All Dataset.ReadWrite.All Gateway.Read.All Gateway.ReadWrite.All Group.Read Group.Read.All Metadata.View_Any Report.Read.All Report.ReadWrite.All StorageAccount.Read.All Workspace.Read.All Workspace.ReadWrite.All";


	private static $resource_url = "https://analysis.windows.net/powerbi/api";
	...
	public static function get_authorization_url( $settings, $antiforgery_id ) {
		...
		// set scopes parameter string
		$arr_scopes = explode(" ", self::$allowed_scopes);

		$sps = "";
		foreach ($arr_scopes as $key => $value) {
			# code...
			if(!empty($value)){
				$sps .= self::$resource_url."/".$value." ";
			}
		}
		...
		$auth_url = $settings->authorization_endpoint . '?'
		 . http_build_query( array(
					...
					'scope'         => 'openid profile '.$sps,
					'login_hint'   => $settings->org_domain_hint,
