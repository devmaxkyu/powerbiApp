...
class Import
{
    ...
    const IMPORT_IN_GROUP_URL = "https://api.powerbi.com/v1.0/myorg/groups/%s/imports";
    ...
    )
    {
        ...
        // Generate the URL
        if($group_id){
            $url = sprintf(self::IMPORT_IN_GROUP_URL, $group_id) . "?nameConflict=$nameConflict&PreferClientRouting=$preferClientRouting";
        }else{
            ...
            $preferClientRouting = 'false')
    {
        ...
        // Generate the URL
        if($group_id){
            $url = sprintf(self::IMPORT_IN_GROUP_URL, $group_id) . "?nameConflict=$nameConflict";
        }else{
            $url = self::IMPORT_URL . "?nameConflict=$nameConflict";
        }
        
        $url .= "&datasetDisplayName=$datasetDisplayName";
        

        $response = $this->client->oneFileUpload($url, $datasetDisplayName, fopen($filepath, 'r'));
        return $this->client->generateResponse($response);
    }
