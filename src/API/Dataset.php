...
class Dataset
{
    ...
    const DELETE_DATASET_URL = 'https://api.powerbi.com/v1.0/myorg/datasets/%s';
    const DELETE_DATASET_IN_GROUP_URL = 'https://api.powerbi.com/v1.0/myorg/groups/%s/datasets/%s';
    ...
    public function createDataset(DatasetModel $dataset, $groupId = null)
    {
        ...
        $response = $this->client->request(client::METHOD_POST, $url, $dataset->getArray());

        return $this->client->generateResponse($response);
    }

    public function deleteDataset($datasetId, $groupId = null){
        
        // Generate the URL
        if($groupId){
            $url = sprintf(self::DELETE_DATASET_IN_GROUP_URL, $groupId, $datasetId);            
        }else{
            $url = sprintf(self::DELETE_DATASET_URL, $groupId);
        }

        $response = $this->client->request(client::METHOD_DELETE, $url, $datasetId);
