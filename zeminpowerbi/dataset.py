...
class Dataset:
    ...
    # relationships - additional feature by zemin
    ...
    def __init__(self, name, dataset_id=None, tables=None, add_rows_api_enabled=None,
                 ...
                 is_effective_identity_roles_required=None, is_on_prem_gateway_required=None, relationships = None):
        self.name = name
        self.id = dataset_id
        self.tables = tables
...
class DatasetEncoder(json.JSONEncoder):
    def default(self, o):
        ...
        if o.relationships is not None:
            relationship_encoder = RelationshipEncoder()
            json_dict[Dataset.relationships_key] = [relationship_encoder.default(x) for x in o.relationships]
...
class Relationship:
    ...
    def __init__(self, name, fromTable, fromColumn, toTable, toColumn, crossFilteringBehavior = None):
        self.name = name
        self.fromTable = fromTable
        self.fromColumn = fromColumn
        self.toTable = toTable
        self.toColumn = toColumn
        self.crossFilteringBehavior = crossFilteringBehavior if crossFilteringBehavior is not None else "bothDirections"
    ...
    def from_dict(cls, dictionary):
        ...
        if Relationship.name_key in dictionary:
            name = str(dictionary[Relationship.name_key])
            # name cannot be whitespace
            if name.isspace():
        ...
        if Relationship.fromTable_key in dictionary:
            fromTable = str(dictionary[Relationship.fromTable_key])
            # fromTable cannot be whitespace
            if fromTable.isspace():
        ...
        if Relationship.fromColumn_key in dictionary:
            fromColumn = str(dictionary[Relationship.fromColumn_key])
            # fromTable cannot be whitespace
            if fromColumn.isspace():
        ...
        if Relationship.toTable_key in dictionary:
            toTable = str(dictionary[Relationship.toTable_key])
            # toTable cannot be whitespace
            if toTable.isspace():
        ...
        if Relationship.toColumn_key in dictionary:
            toColumn = str(dictionary[Relationship.toColumn_key])
            # toColumn cannot be whitespace
            if toColumn.isspace():
        ...
        if Relationship.crossFilteringBehavior_key in dictionary:
            crossFilteringBehavior = str(dictionary[Relationship.crossFilteringBehavior_key])
        else:
            crossFilteringBehavior = "bothDirections"


        return Relationship( name = name,  fromTable = fromTable, fromColumn = fromColumn, 
                            toTable = toTable, toColumn = toColumn, crossFilteringBehavior = crossFilteringBehavior)
