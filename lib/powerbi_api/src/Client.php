...
class Client
{
    ...
    public function request($method, $url, $body = null)
    {
        ...
        if ($body) {
            $requestOptions = array_merge($requestOptions, ['json' => $body]);
        }
     ...
     * @return mixed|\Psr\Http\Message\ResponseInterface
     */
    public function oneFileUpload($url, $filename, $contents)
    {
        // Default Options
        $requestOptions = [
            'headers' => [
        ...
        return $this->httpClient->request(self::METHOD_POST, $url, $requestOptions);
    }
