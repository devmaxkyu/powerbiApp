...
defined( 'AADSSO_DEBUG' ) or define( 'AADSSO_DEBUG', TRUE );
...
require AADSSO_PLUGIN_DIR . '/lib/powerbi_api/vendor/autoload.php';
...
class AADSSO {
	...
	public function __construct( $settings ) {
		...
		add_action( 'aad_sso_login_form', array( $this, 'print_login_link' ) ) ;
		...
		add_action( 'display_reports', array( $this, 'display_reports' ) );
	}
	 ...
	 * @param WP_User|WP_Error $user200
	 *
	 * @return string
	 */
	public function redirect_after_login( $redirect_to, $requested_redirect_to, $user ) {
		...
		echo '<script>alert('.$redirect_to.');</script>';
		return $redirect_to;
	}
	...
	*/
	...
	function authenticate( $user, $username, $password ) {
		...
		if ( isset( $_POST['code'] ) ) {
			...
			$state_is_missing = ! isset( $_POST['state'] );
			$state_doesnt_match = $_POST['state'] != $antiforgery_id;
			...
			$token = AADSSO_AuthorizationHelper::get_access_token( $_POST['code'], $this->settings );
	...
	public function display_reports(){
		...
		if(isset($_SESSION['aadsso_access_token']) && !empty($_SESSION['aadsso_access_token'])){
	        ...
	        $response = $report->getReports($_SESSION['aad_workspace_id']);
	...
	public function initial_pbi_data(){
		...
		if(isset($_SESSION['aadsso_access_token']) && !empty($_SESSION['aadsso_access_token'])){
		    ...
		    $_SESSION['aad_workspace_id'] = $workspace_id;
	        ...
	        $response = $import->ImportFile( AADSSO_PLUGIN_DIR.'/lib/powerbi_api/datasource/cafe.pbix', 'cafe.pbix', $workspace_id);
	        $body = $response->getBody();
	        ...
	        while(True){
		        ...
		        if($array_body->importState == 'Publishing'){
		            ...
		            AADSSO::debug_log('Publishing State');
		            continue;
		        }

		        AADSSO::debug_log('Successed State');
		        break;
	        }
